...
| **Scene stills (duo/multi-character composition)** | Midjourney v6.1 --cref (multi-creature prompt) | $0/img (within quota) | None reliable | — | 🟡 PARTIAL (85-90% consistency issues) | Midjourney is the only tool that reliably composes multiple characters in one frame with scene context. Consistency drifts ~10-15% per generation (proportion variance, color shifts). NEVER use Kontext for multi-object scenes — it fails at spatial placement. Kling 3.0 tested, rejected for wrong aesthetic. Runway tested, rejected as inferior to Seedance. |
...
- **WaveSpeed API intermittent failures:** Experiences connection-refused / timeouts cycling every 5-10 min (likely rate limiting on Bronze tier, 5 videos/min cap). 
  ...
  - **Fallback:** fal.ai Seedance 2.0 (needs credits topped up) or Kling 3.0 (per-use pricing, tested + rejected as inferior aesthetic)
