# Storyboard v59 — Authoring Workflow Architecture Spec v1

**Status:** SUPERSEDED by `STORYBOARD_V59_AUTHORING_WORKFLOW_SPEC_v2.md` (v1 retained as forensic record of Phase 4 first-pass synthesis; do NOT execute against v1 — implementation runs against v2).
**Date:** 2026-05-05 (drafted) / 2026-05-06 (superseded by v2 post-Cursor REJECT-with-changes; v2 → Cursor APPROVE 2026-05-06)
**Branch:** claude/post-redeploy-bug-triage
**Author:** dual-Opus tech-spec session (Agents A + B, orchestrator synthesis), Kim Smith governance
**Supersedes:** none (new architectural spec)
**Superseded by:** `STORYBOARD_V59_AUTHORING_WORKFLOW_SPEC_v2.md` — v2 folds 5 Cursor must-fix items (RR-κ' SOFT alignment, §7.4 SKIP path cleanup, §5.1 sibling audit promotion, §3.0 verifier ops infrastructure, citation lock to `fafdfed`). v2 is the canonical APPROVED spec.
**Note for terminal sessions:** the implementation handoff doc references **v2.md**, not this v1. Phase 6 of the tech-spec session erroneously updated this v1 status header to APPROVED; that update was a process error and is now corrected to SUPERSEDED.
...
All K-finding line citations independently verified against `production_server.py` (commit head at session start) on 2026-05-05.
...
- R-κ' (HARD, captured by `EVENT_1_SHIPS_VIA_SAVED_VIDEO_V1`): Event_1 beat-level state intentionally empty
...
All citations independently verified against `production_server.py` on 2026-05-05.

| # | Finding | Severity | Cite |
|---|---|---|---|
| K1 | `_handle_beat_update_text` HARDCODES `videos.intro` partition. Edits to a beat in `resolution`/`standalone` silently land in `intro`. | HARD | 12048-12054 |
| K2 | `_handle_bg_accept_beats` writes to legacy top-level `state["beats"]`, not v3 `state.videos.<role>.beats`; bypasses `display_order`. Speaker fallback `"Guide Bird"`. | HARD | 8975-9001 (write at 9001), 8990 (fallback) |
| K3 | `_handle_bg_add_beat` HARDCODES `arc_number=1, event_id=2, phase="pre"` — every "+ Add empty beat" goes to Event_2 BG sidecar regardless of scope. | HARD | 9279 |
| K4 | DISPLAY_ORDER_STRICT_V1 prune is asymmetric. Renderer is strict; server-side prune runs only inside `mutate_video_state`. Three handlers call `mutate_state` and bypass the prune. | HARD | 1198-1217 vs 12061, 4079, 9001 |
| K5 | No cross-event move endpoint. No reorder UI on Storyboard tab. Server `_handle_bg_reorder_beats` exists but no client surface. | HARD (move) / SOFT (UI) | grep proof; StoryboardTab.tsx:725-880 |
| K6 | `_assert_event_scope` defaults `allow_missing=True, allow_missing_video_role=True` — scope-less mutations pass through silently. | HARD | 4734-4827 |
| K7 | Accept-All speaker fallback `"Guide Bird"` when sidecar speaker empty. | SOFT | 8990 |
| K8 | Speaker dual-store: `beats[bid].speaker` (top-level/TTS path 3366) AND `beats[bid].phase_1.speaker` (patch_state target 4170-4175). | SOFT | 4170-4175 vs 8990, 12051 |
| D5 | `patch_state._apply` HARDCODES `videos.intro` for image_override, selected_option, trim, pause_after_ms, fade_after_ms, speaker, display_order. Same class as K1, different layer. | HARD | 4083 |
...
**Current state.** Lifecycle handlers scattered: K1 (line 12048), K3 (line 9279), D5 (line 4083). Other partition-touching handlers (`_handle_beat_finalize`, `_handle_beat_use_as_final`, `_handle_beat_delay`, `_handle_beat_trim`, `_handle_select`, `_handle_assign_image`, `_handle_animate`, `_handle_lipsync`) require audit during implementation — the K1/D5 class is structural and these handlers are likely siblings.
...
`_handle_bg_add_beat` derives `(arc_number, event_id, phase)` from `scope.event_id` + `scope.video_role` via mapping `intro→pre, resolution→post, standalone→main` (codified). Hardcode at line 9279 removed.
...
**Current state.** `_handle_bg_accept_beats:9001` calls `mutate_state` with `state.setdefault("beats", {})` — top-level legacy. Bypasses partition + prune. Speaker fallback `"Guide Bird"` at 8990.
...
**Current state.** Two stores: `beats[bid].speaker` (TTS read at 3366) AND `beats[bid].phase_1.speaker` (patch_state target at 4170-4175). `_canonicalize_speaker:745` + `_SPEAKER_ALIAS:3257-3271` already map `"guide bird" → "Chipper"`. Drift is at WRITE-time fallback (K7) and dual-store divergence latent (K8).

**Locked design — two contracts:**

1. **Write-boundary canonicalization (HARD).** Every speaker write site flows through `_canonicalize_speaker(raw)` first. Sites: BG accept-all seed (line 8990), patch_state speaker case (line 4170-4175), graft handler (Pillar 7). The literal `or "Guide Bird"` at line 8990 is dropped.

2. **Single-store contract (DV-2 resolution: top-level canonical).** `partition.beats[bid].speaker` is the canonical write target — it matches the existing TTS reader at line 3366 (fewest downstream code changes) and the conceptual model "speaker is a beat-level property." `phase_1.speaker` becomes a **write-time mirror** for one release as a read-compat shim, then deprecated. Read sites are audited during implementation:
   - TTS read at line 3366: continues to read `beats[bid].speaker` (no change)
...
Rejection reason in body: `code: "graft_pre_render_only"`. This prevents grafting beats whose rendered media files are event-pinned (see RR-1 verification: `_find_beat_audio` looks up TTS in `event_dir / story_scene_tts_v2` — moving a rendered beat across events would silently break audio playback).
...
## 7. Pillar 8 — Salvage decision (EXECUTE)
...
If Kim's prep exceeds the 15-minute timebox, salvage SKIPS:
- Same script with `--mode=clear` flag instead of move map
- Calls `/api/beat/graft` with `move=true` per source beat → `target.position=-1` into a temporary "discard" partition
- Or simpler: a one-shot `clear_event_partition.py` that reads pre-image backup, then `mutate_partition` to set `partition.beats={}` and `partition.display_order=[]` for both Event_1/intro and Event_2/intro
- Audit log row records the skip + Kim's reasoning
...
- `INTRO_LIFT` rules (line 82-86) only relabel WITHIN a state dict
...
**Forensic root cause locked:** pre-LD-456 cross-event Accept-All leak (2026-05-01 origin, documented at `production_server.py:8942` in `_handle_bg_accept_beats` docstring). Server pinned Event_1; client BG context Event_2; `allow_missing=True` let the request through; the seed wrote to top-level legacy `state["beats"]` of the server-pinned event (Event_1). The leak was structurally closed by LD-456 SCOPE_VALIDATION_V1 (C5 ratified). The data damage persisted because:
...
| C-2 | **K1+D5 fix** — `_handle_beat_update_text` + `patch_state._apply` route through `scope_router.mutate_partition`. Hardcoded `videos.intro` literals removed. | `production_server.py:12048-12061, 4079-4185` | TVMC-K1.1, TVMC-D5.1 GREEN | SCOPE_ROUTER_V1 |
| C-3 | **K2+K7 fix** — `_handle_bg_accept_beats` routes through `scope_router.mutate_partition`; partition.beats target; `_canonicalize_speaker` at write boundary; drop `or "Guide Bird"` literal | `production_server.py:8975-9001, 8990` | TVMC-K2.1, TVMC-K7.1, TVMC-K7.2 GREEN | SCOPE_ROUTER_V1, SPEAKER_WRITE_BOUNDARY_CANONICALIZATION_V1 |
| C-4 | **K3 fix** — `_handle_bg_add_beat` derives BG sidecar segment from `(scope.event_id, scope.video_role)`; literal `arc_number=1, event_id=2, phase="pre"` removed; grep CI gate adds the literal-ban | `production_server.py:9279` + grep gate config | TVMC-K3.1 GREEN | BG_HARDCODED_SCOPE_PURGE_V1 |
| C-5 | **K6 fix** — `_assert_event_scope` defaults flip; ~30 caller-site audit; startup assertion lists scope-strict handlers | `production_server.py:4734-4827` + caller sites | TVMC-K6.1 GREEN; existing pathappPatch flow still passes | SCOPE_REQUIRED_DEFAULTS_V1 |
| C-6 | **K8 fix** — speaker dual-store mirror contract; `partition.beats[bid].speaker` canonical; `phase_1.speaker` mirrored on write; `_resolve_beat_speaker` read helper | `production_server.py:4170-4175` + read-site audit | TVMC-K8.1 GREEN | SPEAKER_DUAL_STORE_DEPRECATION_V1 |
...
| C-10 | **K4 fix lands** — `mutate_state` defense-in-depth prune; `DISPLAY_ORDER_STRICT_V2`. **Now safe** because Event_1/intro/beats={} either via salvage (C-9) or via SKIP fallback (C-9b). C-0 snapshot is the rollback safety net. | `production_server.py:1021-1170` (StateManager.mutate_state) | TVMC-K4.1, TVMC-K4.2 GREEN | DISPLAY_ORDER_STRICT_V2 |
...
|---|---|---|---|
| RR-1 | Audio resolution event-pinning could break grafted beats | HARD | Graft handler rejects beats with rendered media (HTTP 400 `graft_pre_render_only`). Kim's salvage scenario is pre-render → unaffected. Follow-on spec for media migration if rendered-beat grafts ever needed. |
...
| RR-κ' | Event_1 contains zero Event_1 content (post-salvage) | HARD | Captured by `EVENT_1_SHIPS_VIA_SAVED_VIDEO_V1` SOFT LD; saved video version is the production deliverable |
...
8. **Audit other partition-touching handlers** (`_handle_beat_finalize`, `_handle_animate`, `_handle_lipsync`, etc.) for K1-class bugs — covered by C-1 grep CI gate but C-2..C-7 commits should explicitly audit each
...
10. **Forensic finding (Q4) is closed** — migration script EXONERATED with code citations; root cause locked.
...
This spec is the Phase 4 synthesis output of the dual-Opus tech-spec session 2026-05-05. It supersedes the Δ-architecture-tighten halt-bundle (commit fafdfed) on `claude/post-redeploy-bug-triage` for the authoring-workflow architecture question.

**Next step:** Phase 5 Cursor review (R-row format per post-redeploy spec pattern). Phase 6 Kim sign-off + atomic-commit handoff doc generation. Implementation in subsequent terminal sessions.
