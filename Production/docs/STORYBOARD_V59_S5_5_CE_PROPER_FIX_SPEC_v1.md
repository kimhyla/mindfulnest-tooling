...
**End of S5.5c+e proper fix spec v1 (Cursor v11 fixes folded).**

This is now ready for terminal execution. The CI workflow installation fix (Phase 1.3 dual-location) + flake governance (§16) + fixture pinning (§17) are the load-bearing additions that distinguish "CI gate that actually works" from "CI gate that decays." Without them, Option 3 is theatrical. With them, Kim's "million more bugs" anxiety has a concrete structural answer: bugs of THIS class are caught at commit time, not at browser smoke time.

No v12 review needed — Cursor v11 amendments are mechanical + the additions are well-scoped governance. Terminal handoff is the next step.
