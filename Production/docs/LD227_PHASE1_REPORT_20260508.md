...
**EXPLICITLY OUT OF SCOPE FOR THIS SESSION** per handoff §What NOT to do.
...
These items are documented in `prod_blockers` (filed as part of Phase 4 prep) — actual blocker filing deferred since this session is overnight autonomous and `prod_blockers` rows can be authored by Kim or in the Phase 4 prep session.
