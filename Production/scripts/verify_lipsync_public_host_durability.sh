...
grep -q 'install_production_server_launchagent.sh' "$START" \
  || fail "start_event_server.sh must delegate to install_production_server_launchagent.sh"
grep -q 'lipsync_public_host.py' "$INSTALL" \
  || fail "install_production_server_launchagent.sh must export lipsync R2 env in launchd plist"
grep -q '\-\-shell-export' "$INSTALL" \
  || fail "install_production_server_launchagent.sh must call lipsync_public_host.py --shell-export"

grep -q 'voiceFirstLipsyncHostBlocked' "$BGTAB" \
  || fail "BgTab must gate Generate when lipsync public host is not ready"
grep -q "effectiveGenerationMode(b, eventId) === 'voice_first'" "$BGTAB" \
...
echo "$accept_block" | grep -q 'SIDECAR_IO_TRANSIENT' \
  || fail "accept-lib-image must return retry_safe transient I/O contention"
! grep -q 'with bg\.sidecar_file_lock' "$BG" \
  || fail "background.py must not call sidecar_file_lock (SQLite cutover P4)"
