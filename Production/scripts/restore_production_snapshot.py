...
def main() -> int:
    ...
    if not args.dry_run:
        try:
            ...
            mig = _SCRIPT.parent / "migrate_sidecar_json_to_sqlite.py"
            json_path = prod / "beat_generator_state.json"
            if mig.is_file() and json_path.is_file():
                proc = subprocess.run(
                    [sys.executable, str(mig), "--source", str(json_path), "--apply"],
                    capture_output=True,
                    text=True,
                )
            ...
            print(f"WARN: SQLite rebuild after restore failed: {exc}", file=sys.stderr)
        print(
