...
def test_dual_prompt_persists_across_mode_toggle():
    sidecar = {
        "arcs": {
            "arc_1": {
                "segments": {
                    "event_2_pre": {
                        "beats": [_dialogue_beat(
                            kling_o3_prompt="Lorelai [warm]: \"Hello\" @Image1 rooted.",
                            kling_o3_prompt_still="STILL INSERT\nHold still.\n",
                        )],
                    },
                },
            },
    ...
    bg.set_beat_generation_mode(
        beat, bg.PIPELINE_MODE_STILL, event_id="2", phase="pre", sidecar=sidecar,
    )
    assert beat["kling_o3_prompt"].startswith("STILL INSERT")
    assert "Hello" not in beat["kling_o3_prompt"]
    bg.set_beat_generation_mode(
        beat, bg.O3_GENERATE_MODE_ELEMENT_NATIVE, event_id="2", phase="pre", sidecar=sidecar,
    )
    assert "STILL INSERT" not in (beat.get("kling_o3_prompt") or "")
    assert "Hello" in (beat.get("kling_o3_prompt") or "")
    bg.set_beat_generation_mode(
        beat, bg.O3_GENERATE_MODE_VOICE_FIRST, event_id="2", phase="pre", sidecar=sidecar,
    )
    assert "Hello" in (beat.get("kling_o3_prompt") or "")


def test_heal_poisoned_still_text_in_o3_prompt():
    """Beat 3/5 class: still instructions overwrote kling_o3_prompt while in voice_first."""
    sidecar = {
        "arcs": {
            "arc_1": {
                "segments": {
                    "event_2_pre": {
                        "beats": [_dialogue_beat(
                            ...
                            speaker="Loral",
                            dialogue_text='Loral: "Is this ancient Everdale?"',
                            o3_generate_mode="voice_first",
                            kling_o3_prompt=(
                                "use pre-made from library; do not submit to Kling O3 Element.\n"
    ...
    beat = sidecar["arcs"]["arc_1"]["segments"]["event_2_pre"]["beats"][0]
    assert bg.heal_beat_dual_prompts(
        beat, sidecar, event_id="2", phase="pre",
    ) is True
    assert "do not submit to Kling O3 Element" in (beat.get("kling_o3_prompt_still") or "")
    assert "do not submit to Kling O3 Element" not in (beat.get("kling_o3_prompt") or "").lower()
    assert "@Image1" in (beat.get("kling_o3_prompt") or "")
    assert "Everdale" in (beat.get("kling_o3_prompt") or "")
