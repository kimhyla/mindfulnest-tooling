...
Stage 9: AUDIO + VIDEO PROD .... Claude Code (ElevenLabs + Vosk + ffmpeg + FLUX Kontext + Seedance/Kling + ByteDance LipSync)
...
### Stage 9: AUDIO + VIDEO PRODUCTION (Claude Code)
...
#### 9B: Video Pipeline (Updated April 10, 2026)
...
| **AUDIO + VIDEO** | Claude Code produces audio + video clips, Kim reviews | ~10 min/module | 5-8 modules |
...
8. **Audio and video are automated but not unreviewed.** Claude Code handles the technical audio and video pipelines (ElevenLabs TTS + FLUX Kontext scene stills + Seedance/Kling animation + ByteDance LipSync), but Kim's listen-through and watch-through is the final quality gate. A technically perfect audio/video file that doesn't FEEL right is not ship-ready.
