...
def _sidecar_with_beat(beat_id: str, *, running: bool = False, job_id: str | None = None) -> dict:
    beat = {
        "beat_id": beat_id,
        "speaker": "Tessa",
...
def test_reconcile_skips_when_subprocess_still_running(tmp_path):
    ...
    sidecar = _sidecar_with_beat(beat_id, running=True, job_id=job_id)
    with patch("beat_generator._PROD_DIR", str(prod)):
        with patch("o3_generation_intent.subprocess_running_for_o3_job", return_value=True):
            closed = intent_mod.reconcile_stale_o3_intent_locks_all_events(sidecar, prod)
    assert closed == 0
    assert intent_mod.beat_has_active_intent(beat_id, event_dir) is True
...
def test_reconcile_skips_active_pipeline_log_even_when_beat_approved(tmp_path):
    ...
    assert closed == 0
    assert intent_mod.beat_has_active_intent(beat_id, event_dir) is True
    terminal = intent_mod.load_intent_terminal(intent_mod.terminal_path_for_job(job_id, event_dir))
...
def test_reconcile_closes_stale_o3_submit_orphan_after_grace(tmp_path):
    """Regression: dead subprocess stuck at o3_submit must not block resubmit forever."""
    prod = tmp_path / "Production"
    event_dir = prod / "Event_2"
    jobs = event_dir / "arlo_o3_jobs"
    jobs.mkdir(parents=True)
    ...
    log_path.write_text(
        ...
        + json.dumps({"phase": "o3_submit", "beat_id": beat_id}) + "\n"
        + "[sidecar_lock] waiting for beat_generator_state.json.lock\n",
        encoding="utf-8",
    )
    import os
    import time
...
def test_reconcile_does_not_instant_done_new_intent_on_approved_beat(tmp_path):
    """Regression: submit reconcile must not terminal-done a fresh job using old g9."""
    prod = tmp_path / "Production"
    event_dir = prod / "Event_2"
    jobs = event_dir / "arlo_o3_jobs"
    jobs.mkdir(parents=True)
    beat_id = "bg_arc1_event2_pre_beat_28"
    job_id = "ca5a68f8"
    video = event_dir / "kling_o3_clips" / f"{beat_id}_g9_element_o3_master_delivery.mp4"
    video.parent.mkdir(parents=True)
    video.write_bytes(b"mp4")
    log_path = jobs / f"{job_id}_{beat_id}.log"
    log_path.write_text(
...
def test_session_state_handler_does_not_reconcile_intent_on_get():
    src = (
