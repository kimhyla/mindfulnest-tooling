...
def test_submit_handler_uses_generation_intent_commit():
    ...
    submit_block = text.split("def handle_bg_submit_arlo_o3_voice", 1)[1].split("\ndef handle_bg_", 1)[0]
    assert "stamp_o3_prompt_box_law" in submit_block
    assert "ensure_operator_insert_char_ref_parity" not in submit_block
...
def test_still_to_o3_flip_does_not_rewrite_prompt(monkeypatch):
    ...
    assert "Oh. My. Gosh." in (beat.get("kling_o3_prompt") or "")
