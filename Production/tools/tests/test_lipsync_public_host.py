...
def test_start_event_server_exports_lipsync_env():
    ...
    install = (scripts / "install_production_server_launchagent.sh").read_text(encoding="utf-8")
    assert "lipsync_public_host.py" in install
    assert "--shell-export" in install
    start = (scripts / "start_event_server.sh").read_text(encoding="utf-8")
    assert "install_production_server_launchagent.sh" in start
