...
def test_gpt_batch_job_reconciled_from_server_beats() -> None:
    nav = NAV_STATUS.read_text(encoding="utf-8")
    assert "export function collectActiveGptBatchJobFromBeats" in nav
    assert "stills_pending" in nav.split("collectActiveGptBatchJobFromBeats", 1)[1].split("\nexport ", 1)[0]
    bg = BG_TAB.read_text(encoding="utf-8")
    assert "collectActiveGptBatchJobFromBeats" in bg
    assert "setActiveJobId(nextGptJob)" in bg or "setActiveJobId(initialGptJob)" in bg
