...
def test_session_state_skips_full_migrate_and_wavespeed_heal():
    ...
    assert "_migrate_sidecar(sidecar)" not in block
    ...
    assert "reconcile_refer_if_pose_hash_matches" not in block


def test_session_state_disk_reconcile_force_only():
    block = _handler_block("handle_bg_session_state")
    assert "if force_reconcile_o3:" in block
    assert "reconcile_kling_o3_sidecar(sidecar, event_dir)" in block
...
def test_submit_uses_fast_sidecar_read_and_update_beat_locked_for_pid():
    ...
    assert "sidecar_file_lock(timeout_s=30)" in block
    assert "ensure_sidecar_schema_defaults(sidecar)" in block
...
def test_ensure_o3_job_metadata_throttled_and_timeout_lock():
    ...
    assert "sidecar_file_lock(timeout_s=5)" in block
...
def test_delivery_encode_uses_medium_preset():
    ...
    block = text.split("def encode_delivery_video", 1)[1].split("\ndef ", 1)[0]
