...
class TestIntroDefaultWhoosh(unittest.TestCase):
    def test_apply_whoosh_at_tail_on_new_intro(self):
        from server_handlers.stitch_editor import apply_stitch_intro_default_whoosh_cue
        ...
        with unittest.mock.patch(
            ...
            self.assertTrue(apply_stitch_intro_default_whoosh_cue(h, slot))
    ...
    def test_apply_whoosh_skips_when_cue_already_present(self):
        ...
        self.assertFalse(apply_stitch_intro_default_whoosh_cue(h, slot))
        self.assertEqual(len(slot["sfx_cues"]), 1)
