...
def test_resolve_element_native_default_for_event2_pre_speak_beat():
...
def test_resolve_voice_first_when_segment_override_legacy():
    sidecar = _sidecar_event2_pre()
...
def test_resolve_element_native_without_spoken_dialogue():
    sidecar = _sidecar_event2_pre()
    beat = sidecar["arcs"]["arc_1"]["segments"]["event_1_pre"]["beats"][0]
    beat.pop("dialogue_text", None)
    beat["kling_o3_prompt"] = "Scene pans across the meadow."
...
def test_submit_handler_selects_avatar_script_for_default_speak():
