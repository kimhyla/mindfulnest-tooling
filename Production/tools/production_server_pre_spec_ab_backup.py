...
class ProductionHandler(BaseHTTPRequestHandler):
    ...
    def _handle_bg_update_beat(self, body: dict) -> None:
        """POST /api/bg/update-beat {beat_id, [field...]} → { ok }"""
    ...
    def _handle_bg_reorder_beats(self, body: dict) -> None:
        """POST /api/bg/reorder-beats {beat_ids: [...]} → { ok }"""
    ...
    def _handle_bg_accept_beats(self, body: dict) -> None:
        """POST /api/bg/accept-beats {beats, segment} → { ok }
    ...
    def _handle_bg_accept_option(self, body: dict) -> None:
        ...
        with bg._sidecar_lock:
            ...
            # If the option has a downloaded video (Kling path), record it for group assembly
            for opt in beat.get("flux_options", []) or []:
                if not opt:
                    continue
    ...
    def _handle_cr_save_crop(self, body: dict) -> None:
        """POST /api/cr/save-crop {crop_png_b64, beat_id, source_key}
...
"""
    ...
    def _serve_beat_audio(self, beat_id: str) -> None:
        ...
        audio_path = _find_beat_audio(self.app.event_dir, beat_id)
    ...
    def _handle_beat_update_text(self, body: dict) -> None:
        ...
        tts_exists = _find_beat_audio(self.app.event_dir, beat_id) is not None
        ...
        try:
            ...
            try:
                ...
                for meta in beat_meta:
                    ...
                    # Preflight 111 fix: rebuild if normalized cache is older
                    # than the source file. Previously we only checked existence,
                    # so a lipsync rescue that updated animation_clips/*.mp4
                    # would not invalidate preview/normalized/*. Symptom: Kim
                    # hit Preview All Stitched after a lipsync completion and
                    # saw the OLD rendered output because the normalized
                    # intermediate was stale.
                    needs_rebuild = (
                        not norm.is_file()
                    ...
                    trimmed = trimmed_dir / f"{bid}_trimmed.mp4"
    ...
    _TIMELINE_SEGMENT_LABELS: list[str] = ["Intro", "Phase A", "Phase B", "Win"]
    ...
    def _handle_timeline_audio(self, event_id: str) -> None:
        ...
        Extract audio from stitched preview; cache by (path+mtime) hash.
        Returns JSON: {audio_url, duration_ms, segment_boundaries}.
        SIZE_BUDGET_AUDIO_V1: AAC 128kbps mono 44.1kHz.
        """
        import hashlib as _hl  # noqa: PLC0415
        preview_mp4 = self._get_current_preview_mp4()
        if not preview_mp4:
            return self._send_json(404, {
                "error": "Build Preview first",
                "hint": "Click 'Preview-Stitched v2' to generate the preview, then open the timeline.",
            })

        mtime_ms = int(preview_mp4.stat().st_mtime * 1000)
        cache_key = _hl.md5(
            f"{preview_mp4}:{mtime_ms}".encode(), usedforsecurity=False
        ).hexdigest()[:16]
        ...
        segment_boundaries = self._compute_timeline_segment_boundaries(total_ms)
    ...
    def _compute_timeline_segment_boundaries(self, total_ms: int) -> list:
        """Compute 4 segment boundaries by ffprobing each beat's selected clip."""
        state = self.app.state.read_state()
        beats = state.get("beats", {})
        display_order = state.get("display_order") or sorted(beats.keys())
        clips_dir = self.app.state.clips_dir
        ...
        for bid in display_order:
            beat = beats.get(bid, {})
            p1 = beat.get("phase_1", {})
            sel = p1.get("selected_option")
            if sel is None:
                continue
            opts = p1.get("options", [])
            if not (0 <= sel < len(opts)):
                continue
            clip_rel = opts[sel].get("file", "")
            if not clip_rel:
                continue
            ...
            try:
                ...
                d_ms = int(float(json.loads(dp.stdout)["format"]["duration"]) * 1000)
                beat_durations.append(d_ms)
            except Exception:
                beat_durations.append(0)

        labels = self._TIMELINE_SEGMENT_LABELS
        n = len(beat_durations)
        if n == 0:
            q = max(1, total_ms // 4)
            return [
                {"label": lbl, "start_ms": i * q, "end_ms": min((i + 1) * q, total_ms)}
        ...
        for i, lbl in enumerate(labels):
            start_ms = cumulative
            chunk = beat_durations[i * chunk_size: (i + 1) * chunk_size]
            seg_ms = sum(chunk)
            cumulative += seg_ms
            boundaries.append({"label": lbl, "start_ms": start_ms, "end_ms": cumulative})
        if boundaries and total_ms > 0:
            boundaries[-1]["end_ms"] = total_ms
        return boundaries
    ...
    def _handle_timeline_preview_with_sfx(self, body: dict) -> None:
        ...
        if not preview_mp4:
            return self._send_json(404, {
                "error": "Build Preview first",
                "hint": "Click 'Preview-Stitched v2' before mixing SFX.",
            })
