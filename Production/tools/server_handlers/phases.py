...
def sweep_phase_a_lipsync_resume(state_mgr) -> None:
    """Resume Phase A ByteDance base-clip jobs after server restart.
...
def reconcile_phase_a_layered_lipsync(state_mgr, client) -> None:
    """Optional resume for Phase A layered jobs (not the Send default).

    Kept for durability of in-flight layered experiments. Production Send uses
    ByteDance + sweep_phase_a_lipsync_resume. Opt-in via MN_PHASE_A_LAYERED=1
    in the poller if a layered job must be reconciled.
    """
    if client is None:
        return
    snap = state_mgr.read_state()
...
def handle_phase_a_lipsync(h, body: dict) -> None:
    """POST /api/phase_a/lipsync — ByteDance LatentSync on pinned Arlo desk idle.
