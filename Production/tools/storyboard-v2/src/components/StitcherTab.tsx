...
export function StitcherTab() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${SERVER_BASE}/api/stitch_editor/library`);
        if (cancelled) return;
        if (!res.ok) {
        ...
        // Pick the active job for this event (job_name pattern: phase_*_<event>).
        const jobs = data.jobs ?? [];
        const eventJob = jobs.find((j) => j.job_name?.includes(eventName)) ?? jobs[0] ?? null;
        setJob(eventJob);
        if (eventJob?.slots) {
          const slotKeys = Object.keys(eventJob.slots);
  ...
  const onBake = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      setStatusMsg(res.ok ? `✓ Baked: ${(data as { bake_path?: string }).bake_path ?? job.job_name}` : `✗ Bake HTTP ${res.status}`);
    ...
    <section class="mn-tab-pane mn-stitcher-pane" data-testid="pane-stitcher">
      ...
      {loading ? (
        ...
        <div class="mn-empty" data-testid="stitcher-error">
          <p class="mn-warn">Could not reach /api/stitch_editor/library.</p>
      ...
      <footer class="mn-pane-footer">
        <div class="mn-stitcher-bake-row">
          <button
            ...
            onClick={onBake}
            disabled={!job?.job_name || busySlot !== null}
          >
            🔨 Bake final MP4
          </button>
          {job?.bake_path ? (
