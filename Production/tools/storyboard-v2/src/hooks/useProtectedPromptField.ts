...
export function useProtectedPromptField({
  ...
  useLayoutEffect(() => {
    ...
    if (domText === externalText) {
      textRef.current = externalText;
      lastSavedRef.current = externalText;
      return;
    }
    // Prompt-box is law: never clobber a longer in-box prompt with stale server shorthand
    // (session refresh / beat-list mutations must not snap back to an old saved stub).
    if (domText.trim().length > externalText.trim().length) {
      textRef.current = domText;
    ...
    textRef.current = externalText;
    lastSavedRef.current = externalText;
  }, [externalText, fieldId, isEditing, lockedExternal, publishRegistry]);
