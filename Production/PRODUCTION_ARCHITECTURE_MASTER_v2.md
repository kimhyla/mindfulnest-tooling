...
**Note on batching (April 14, 2026):** The SPEED-* optimization decisions (18 entries in Directus `prod_session_decisions`) establish arc-level batching by work type for M2-M6. M1 is the **pilot module** — it follows the Phase 1-4 order below to prove the pipeline. Once M1 is complete, M2-M6 use **batching by work type** (NOT the Phase 1-4 order): (1) Write all Phase B scripts in shared context → (2) Kim reviews all Phase Bs → (3) Generate all images + Phase A scripts in parallel → (4) Kim reviews all storyboards, assigns images via unified image selector/cropper → (5) Produce all audio mixes → (6) Kim does all listen-throughs. This cuts calendar time from 6-8 weeks to ~4 weeks per arc. See `.auto-memory/project_speed_optimization_plan.md` for the full batching schedule.

**Completed tools (as of April 14, 2026):** Unified Image Selector + Cropper (`build_image_selector_cropper.py`) — replaces per-image cropper workflow for M2+ production. QA Validator Suite (`Production/validators/`) — integrated into pipeline.py. Both registered in Directus and documented in PIPELINE_BRAIN.
